...
public class RequestProcessor implements Runnable{
	...
	private void refuseReceiveFile(Request request) throws IOException {
		...
		ResponseStatus.Response response = new ResponseStatus.Response();  //创建一个响应对象
	...
	private void agreeReceiveFile(Request request) throws IOException {
		...
		//向接收方发出接收文件的响应
		ResponseStatus.Response response2 = new ResponseStatus.Response();  //创建一个响应对象
	...
	public void login(OnlineClientIOCache currentClientIO, Request request) throws IOException {
		...
		if(null != user){
			if(DataBuffer.onlineUsersMap.containsKey(user.getId())){ //用户已经登录了
				...
				//通知其它用户有人上线了
				ResponseStatus.Response response2 = new ResponseStatus.Response();
	...
	public void chat(Request request) throws IOException {
		...
		ResponseStatus.Response response = new ResponseStatus.Response();
		response.setStatus(ResponseStatus.OK);
	...
	/** 给所有在线客户都发送响应 */
	private void iteratorResponse(ResponseStatus.Response response) throws IOException {
	...
	/** 向指定客户端IO的输出流中输出指定响应 */
	private void sendResponse(OnlineClientIOCache onlineUserIO, ResponseStatus.Response response)throws IOException {
		ObjectOutputStream oos = onlineUserIO.getOos();
		oos.writeObject(response);
		oos.flush();
	}
